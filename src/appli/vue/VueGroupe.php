...
class VueGroupe {
    ...
    public function choixAffichage($sel){
        switch ($sel){
            case 1 :
                $text = creerGroupe();
            break;
	...
	public function creerGroupe($idAdmin){
        $tab = Groupe::add($idAdmin, 'false', null, ...);
		$html = detailGroupe($idAdmin);
		
		return $html;
    }
	
	public function detailGroupe($id){
		$control = new UtilisateurController;
		$control->affichListeUtil();
        $html = '';
        foreach(Groupe::get($id) as $util){
            $html+= appli\controller\UtilisateurController->affichUtil($id);
        }
        return $html;
    }
